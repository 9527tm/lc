...
class Solution {
    ...
    //https://leetcode.com/problems/find-duplicate-file-in-system/discuss/104120
    //https://leetcode.com/problems/find-duplicate-file-in-system/discuss/184116
    public List<List<String>> findDuplicate(String[] paths) {
        return sol1(paths);
    }
    
    private List<List<String>> sol1(String[] paths) {
        Map<String, List<String>> map = new HashMap<>();
        for (String path : paths) {
            String[] entries = path.split("\\ "); //H.W.: single backslash in regex
            String dir = entries[0] + "/";
            for (int i = 1; i < entries.length; i++) {
                int leftIndex = entries[i].indexOf('(');
                int rightIndex = entries[i].length() - 1;
                String fullName = dir + entries[i].substring(0, leftIndex);
                String content = entries[i].substring(leftIndex + 1, rightIndex);
        ...
        List<List<String>> res = new ArrayList<>(); //H.W.: forgot to select duplicates only
        for (List<String> list : map.values()) {    //      [root/a 1.txt(abc) 2.txt(efg)]
            if (list.size() > 1) {
                res.add(list);
            }
        }
